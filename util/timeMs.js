...
module.exports = (Time) => {
	...
	if (week !== undefined) return calculateWeek();

	return Args[0].includes("/") || Args[0].includes(":")
		? withDay()
		: minimalDay();
	...
	function minimalDay() {
		for (let i = 0; i < Args.length; i++) {
			if (
				["a", "y"].includes(Args[i].at(-1)) ||
				["ano", "year", "anos", "y"].includes(Args[i + 1])
			) {
				const string = ["a", "y"].includes(Args[i].at(-1))
					? `${Args[i]}`
					: `${Args[i]}${Args[i + 1]}`;
			...
			if (
				Args[i].at(-1).includes("d") ||
				["dias", "dia", "day", "days", "d"].includes(Args[i + 1])
			) {
				const string = Args[i].at(-1).includes("d")
					? `${Args[i]}`
					: `${Args[i]}${Args[i + 1]}`;
			...
			if (
				Args[i].slice(-1).includes("h") ||
				["horas", "hora", "hour", "hours", "h"].includes(Args[i + 1])
			) {
				const string = Args[i].at(-1).includes("h")
					? `${Args[i]}`
					: `${Args[i]}${Args[i + 1]}`;
			...
			if (
				Args[i].slice(-1).includes("m") ||
				["minuto", "minutos", "minute", "minutes", "m"].includes(Args[i + 1])
			) {
				const string = Args[i].at(-1).includes("m")
					? `${Args[i]}`
					: `${Args[i]}${Args[i + 1]}`;
			...
			if (
				Args[i].slice(-1).includes("s") ||
				["segundo", "segundos", "second", "seconds", "s"].includes(Args[i + 1])
			) {
				const string = Args[i].at(-1).includes("s")
					? `${Args[i]}`
					: `${Args[i]}${Args[i + 1]}`;
	...
	function withDay() {
		...
		if (["tomorrow", "amanhã"].includes(data.toLowerCase())) data = day(true);

		if (["hoje", "today"].includes(data.toLowerCase())) data = day();
		...
		if (data.includes("/") && data.length === 10 && !hour) hour = "12:00";
	...
	function calculateWeek() {
		...
		Args = `${date.toLocaleDateString("pt-BR")} ${
			Args.slice(1).join(" ") || "12:00"
		}`.split(/ +/g);
		return withDay();
	}
};
